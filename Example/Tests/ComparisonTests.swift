...
class ComparisonTests: XCTestCase {
    ...
    func testLessThanComparison() {
        do {
            let query = Query<TestEntity>(in: kangaroo.backgroundContext).filtered { \TestEntity.id < 4 }
        ...
        do {
            ...
            let query = Query<TestEntity>(in: kangaroo.backgroundContext).filtered { \TestEntity.id < optional }
            let entities = query.execute()
            XCTAssertEqual(entities.count, 2)
        }
    }
    ...
    func testLessOrEqualsThanComparison() {
        do {
            let query = Query<TestEntity>(in: kangaroo.backgroundContext).filtered { \TestEntity.id <= 5 }
            let entities = query.execute()
